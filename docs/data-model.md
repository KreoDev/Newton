...
- QR codes and vehicle disks are stored as encrypted strings for security.
...
```javascript
{
  ...
  truckQR: { status: "valid", data: "encrypted_data" },
  truckDisk: { status: "valid", expiry: "2024-06-30" },
  trailer1QR: { status: "valid", data: "encrypted_data" },
...
```javascript
{
  permissions: {
    ...
    "admin.serviceProviders": { description: "Manage service providers" },
    ...
    "admin.locations": { description: "Manage locations" },
...
Key Firestore composite indexes needed:

1. **orders**: (status, clientId, createdAt DESC)
...
- All documents should validate required fields
- Users can only modify their own notification preferences
- Service providers can only see their own orders/assets
- Weighbridge operations require specific permissions
- Audit logs are write-only for regular users
- Settings documents require admin permissions
