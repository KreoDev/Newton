...
const PLACEHOLDER_GROUPS = {
  ...
  Other: [{ key: "{{reason}}", value: "Maintenance required", description: "Reason for action" }],
}
...
export function TemplateEditor({ open, onClose, onSuccess, template }: TemplateEditorProps) {
  ...
  const handleResetToDefault = async () => {
    const confirmed = await showConfirm("Reset Template", "Are you sure you want to reset this template to its default values? This will discard all changes.", "Reset")
    if (!confirmed) return
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>{isEditing ? "Update notification template with live preview" : "Add a new notification template with live preview"}</DialogDescription>
        </DialogHeader>
          ...
          <Tabs defaultValue="editor" className="w-full">
            ...
            <TabsContent value="editor" className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={category} onValueChange={setCategory}>
                    <SelectTrigger id="category" className="mt-2 w-full glass-surface border-[var(--glass-border-soft)] shadow-[var(--glass-shadow-xs)] bg-[oklch(1_0_0_/_0.72)] backdrop-blur-[18px]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {CATEGORIES.map(cat => (
              ...
              <div className="space-y-2">
                ...
                <Input id="subject" value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g., New Asset Added - {{assetType}}" required />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="body" value={body} onChange={e => setBody(e.target.value)} placeholder="Enter email body with placeholders..." rows={8} required />
              </div>
                    ...
                    <div key={groupName} className="mb-4 last:mb-0">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {placeholders.map(placeholder => (
                          <Button key={placeholder.key} type="button" variant="outline" size="sm" onClick={() => insertPlaceholder(placeholder.key)} className="text-xs" title={placeholder.description}>
                            {placeholder.key}
                          </Button>
                        ))}
                      </div>
              ...
              <div className="border rounded-md p-6 bg-muted/20 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-4 bg-background border rounded-md min-h-[300px] max-h-[400px] overflow-y-auto">
                    <div className="prose prose-sm max-w-none">{previewBody ? <pre className="whitespace-pre-wrap font-sans text-sm">{previewBody}</pre> : <p className="text-muted-foreground italic">Email body will appear here...</p>}</div>
                  </div>
                </div>
